    ...
    <MediaQuery minWidth={1224}>
      ...
      <div key={propDatas.id} className="movie-card" ref={nextPage}>
        ...
        <div className="movie-card__info">
          ...
          id={propDatas.id} onClick={(e)=>addFavoriteMoviete(e)} ><h4 id={propDatas.id}>收藏</h4></button>:null}
        </div>   
      </div>: 
      <div key={propDatas.id} className="movie-card">
        ...
        <div className="movie-card__info">
          ...
          onClick={pageLink} id={propDatas.id}><h4>詳細</h4></button>
          {userData.id.length !== 0 ? <button className="movie-card__info__add-button"
          onClick={(e)=>{addFavoriteMoviete(e)}} id={propDatas.id}><h4 id={propDatas.id}>收藏</h4></button>:null}
        </div>   
      </div>}
      </MediaQuery>
      </Fragment>
